...
export const Carousel = ({ items, initialScroll = 0 }: CarouselProps) => {
  ...
  const handleCardClose = (index: number) => {
    if (carouselRef.current) {
      const cardWidth = 272 // Width of our cards
      const gap = 16 // Assuming a gap of 16px between cards
          ...
          <div className='mx-auto flex max-w-7xl flex-row justify-start gap-4 pl-4'>
            {items.map((item, index) => (
              <motion.div
                ...
                key={`card-${(item.props as { card: Card }).card.id}`}
  ...
  return (
    <>
      ...
      <motion.button
        layoutId={layout ? `card-${card.id}` : undefined}
        onClick={handleOpen}
        className='relative z-10 flex flex-col items-start justify-start overflow-hidden rounded-3xl bg-gray-100 dark:bg-neutral-900'
        style={{ width: '272px', height: '770px' }}
      >
